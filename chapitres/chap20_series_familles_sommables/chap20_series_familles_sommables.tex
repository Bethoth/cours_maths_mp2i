...
\begin{prop}\thlabel{prop:sommesInfiniesExponentielleCosEtSin}
...
\begin{defi}[Somme d'une famille sommable]
...
\begin{prop}[Invariance de la somme par permutation]\thlabel{prop:invarianceDeLaSommeParPermutationCasComplexe}
...
\begin{cor}[Application aux séries absolument convergentes]
...
\begin{rem}
La proposition suivante généralise la \thref{prop:invarianceDeLaSommeParPermutationCasComplexe}.
\end{rem}
...
\begin{cor}\thlabel{cor:produitDeFamillesSommablesEstSommable}
...
\begin{defi}[Produit de Cauchy de deux séries]
...
\begin{rem}
\begin{itemize}
    \item On suppose que les séries sont définies \guillemets{à partir de \(n=0\)}. Quand ce n'est pas le cas, le plus simple est d'ajouter des termes nuls à la série pour manipuler proprement les produits de Cauchy. \\
...
Selon le \thref{cor:produitDeFamillesSommablesEstSommable}, la famille \(\paren{x_iy_j}_{\paren{i,j}\in\N^2}\) est sommable et de somme : \[\begin{aligned}
...
\begin{defprop}[Fonction exponentielle]\thlabel{defprop:fonctionExponentielle}
Soit \(z\in\C\).
...
\begin{dem}
Selon la \thref{prop:sommesInfiniesExponentielleCosEtSin} : \[\sum_{n=0}^{\pinf}\dfrac{\abs{z}^n}{n!}=\e{\abs{z}}<\pinf.\]

Donc \(\sum_n\dfrac{\abs{z}^n}{n!}\) converge donc \(\sum_n\dfrac{z^n}{n!}\) converge absolument.
\end{dem}

\begin{prop}\thlabel{prop:propriétésExponentielleDéfinieCommeLaSommeDUneSérie}
On a :
...
\begin{rem}[Compatibilité avec les anciennes définitions]
\begin{enumerate}
    \item D'après la \thref{prop:sommesInfiniesExponentielleCosEtSin}, la \thref{defprop:fonctionExponentielle} est bien compatible avec la fonction \(\exp:\R\to\R\) vue au lycée. \\
    \item On avait déjà défini une fonction \(\exp:\C\to\C\) en posant : \[\quantifs{\forall a,b\in\R}\e{a+\i b}=\e{a}\e{\i b}=\e{a}\paren{\cos b+\i\sin b}.\] La \thref{defprop:fonctionExponentielle} est également compatible avec cette définition. \\
    \item Vous aurez en deuxième année des outils pour étudier les \guillemets{séries de fonctions} qui rendront cette définition davantage exploitable.
\end{enumerate}
\end{rem}
...
D'après la \thref{prop:sommesInfiniesExponentielleCosEtSin}, on a : \[\begin{WithArrows}
