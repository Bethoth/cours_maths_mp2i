...
On a : \[u_n\simqd{n\to\pinf}v_n\imp u_{\phi\paren{n}}\simqd{n\to\pinf}v_{\phi\paren{n}}.\]
\end{prop}
...
L'affirmation \guillemets{donc \(\ln\paren{1+\e{x}}\simqd{x\to\pinf}\e{x}\)} est fausse car \(\e{x}\tendqd{x\to\pinf}\pinf\not=0\).
...
\section{Méthodes à bien connaître}
...
\subsection{Se ramener aux développements limités connus}
